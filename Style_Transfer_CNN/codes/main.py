...
def preprocess_direct_image(image_tensor, target_height, target_width):
    # Resize image to target size
    image_tensor = tf.image.resize(image_tensor, (target_height, target_width))
    
    # Convert image to array
    arr = keras.preprocessing.image.img_to_array(image_tensor)
    arr = np.expand_dims(arr, axis=0)
    
    # Tạo bản sao có thể ghi của mảng
    arr_copy = arr.copy()
    
    # Preprocess for VGG19
    arr_copy = vgg19.preprocess_input(arr_copy)
    
    return tf.convert_to_tensor(arr_copy)
...
def compute_total_variation_loss(image):
    # Tính total variation loss để làm mịn ảnh
    a = tf.square(image[:, :result_height - 1, :result_width - 1, :] - image[:, 1:, :result_width - 1, :])
    b = tf.square(image[:, :result_height - 1, :result_width - 1, :] - image[:, :result_height - 1, 1:, :])
    return tf.reduce_sum(tf.pow(a + b, 1.25)) / tf.cast(tf.size(image), tf.float32)

'''if __name__ == "__main__":
    # Prepare content, stlye images
    #path = os.path.abspath(os.getcwd())
    current_folder = os.getcwd()
    path = current_folder.replace('\\', '/')
    ...
    content_image_path = path + '/codes/dataset/HCM.jpg'
    style_image_path = path + '/codes/dataset/starry_night_small.jpg'
    ...
    # Khởi tạo generated_image bằng cách trộn lẫn ảnh nội dung và ảnh style
    content_weight = 0.5  # Tùy chỉnh trọng số của ảnh nội dung
    style_weight = 1 - content_weight  # Tùy chỉnh trọng số của ảnh style
    generated_image = tf.Variable((content_weight * content_tensor) + (style_weight * style_tensor))
    ...
    for iter in range(NUM_ITER):
        with tf.GradientTape() as tape:
            ...
            loss += compute_total_variation_loss(generated_image)
        ...
        if (iter + 1) % 5 == 0:
            name = path + "/codes/result/generated_at_iteration_%d.png" % (iter + 1)
            save_result(generated_image, result_height, result_width, name)

    name = path + "/codes/result/result_%d_%f_%f.png" % (NUM_ITER, CONTENT_WEIGHT, STYLE_WEIGHT)
    save_result(generated_image, result_height, result_width, name)
'''
...
def generate_pyramid(image, levels):
    ...
    for i in range(levels - 1):
        # Sử dụng LANCZOS thay vì ANTIALIAS
        new_width = image.size[0] // 2**i
        new_height = image.size[1] // 2**i
        resized_image = image.resize((new_width, new_height), Image.LANCZOS)
        pyramid.append(resized_image)
    return pyramid[::-1]  # Reverse to start with the smallest image
...
if __name__ == "__main__":
    ...
    content_pyramid = generate_pyramid(content_image, pyramid_levels)
    style_pyramid = generate_pyramid(style_image, pyramid_levels)
    ...
    # Optimize result image using pyramids
    for level in range(pyramid_levels):
        # Resize images to current pyramid level
        curr_content = tf.image.resize(content_pyramid[level], (result_height, result_width))
        curr_style = tf.image.resize(style_pyramid[level], (result_height, result_width))
        
        # Preprocess images
        content_tensor = preprocess_direct_image(curr_content, result_height, result_width)
        style_tensor = preprocess_direct_image(curr_style, result_height, result_width)

        # Update generated image size
        generated_image = tf.Variable(tf.image.resize(generated_image, (result_height, result_width)))
        ...
        for iter in range(NUM_ITER):
            ...
            if (iter + 1) % 5 == 0:
                name = path + "/codes/result/generated_at_iteration_" + str(iter + 1) + "_at_level_" + str(level + 1) + ".png"
                save_result(generated_image, result_height, result_width, name)
